...
class IDAM(nn.Module):
    ...
    def forward(self, x):
        ...
        ##########################################################################
        New_RGB_A = T * add_gate + RGB
        New_T_A = RGB * add_gate + T
        ...
        ##########################################################################
        New_RGB = New_RGB_A * rgb_gate
        New_T = New_T_A * t_gate
        ##########################################################################
        ...
        New_fuse = New_RGB * attention_vector_l + New_T * attention_vector_r
...
class aggregation_init(nn.Module):
    def __init__(self, channel,mode='None'):
        ...
        if mode == 'out':
            self.conv = nn.Conv2d(3*channel, 9, 1)
        if mode == 'splat':
...
class FA_encoder(nn.Module):
    def __init__(self,dropout_rate: float = .09):
        ...
        ######## IDAM ################
        self.idam_0 = IDAM(64)
        self.idam_1 = IDAM(256)
        self.idam_2 = IDAM(512)
        self.idam_3 = IDAM(1024)
        self.idam_4 = IDAM(2048)
        ########  RGB ENCODER  ########
    ...
    def forward(self, input):
        ...
        ######################################################################
        in_rgbt_0 = torch.cat([rgb,thermal],dim=1)
        out_rgb = self.idam_0(in_rgbt_0)
        rgb = out_rgb[:, 0:64, :]
        thermal = out_rgb[:, 64:128, :]
        fuse = out_rgb[:, 128:64*3, :]
        ######################################################################
        # layer1
        ######################################################################
        ...
        in_rgbt_1 = torch.cat([rgb_1, thermal_1], dim=1)
        out_fuse_1 = self.idam_1(in_rgbt_1)
        rgb_1 = out_fuse_1[:, 0:256, :]
        thermal_1  = out_fuse_1[:, 256:512, :]
        fuse_1 = out_fuse_1[:, 512:768, :]
        ######################################################################
        # layer2
        ######################################################################
        rgb_2_1 = self.encoder_rgb_layer2(rgb_1)
        if verbose: print("rgb.size() after layer2: ", rgb_2_1.size())  # (60, 80)
        thermal_2_1 = self.encoder_thermal_layer2(thermal_1)
        if verbose: print("thermal.size() after layer2: ", thermal_2_1.size())  # (60, 80)
        in_rgbt_2 = torch.cat([rgb_2_1, thermal_2_1], dim=1)
        out_fuse_2 = self.idam_2(in_rgbt_2)
        rgb_2_1 = out_fuse_2[:, 0:512, :]
        thermal_2_1 = out_fuse_2[:, 512:1024, :]
        fuse_2_1 = out_fuse_2[:, 1024:512*3, :]
        ######################################################################
        # layer3
        ######################################################################
        rgb_3_1 = self.encoder_rgb_layer3(rgb_2_1)
        if verbose: print("rgb.size() after layer3: ", rgb_3_1.size())  # (30, 40)
        thermal_3_1 = self.encoder_thermal_layer3(thermal_2_1)
        if verbose: print("thermal.size() after layer3: ", thermal_3_1.size())  # (30, 40)
        in_rgbt_3 = torch.cat([rgb_3_1, thermal_3_1], dim=1)
        out_fuse_3 = self.idam_3(in_rgbt_3)
        ...
        in_rgbt_4 = torch.cat([rgb_4_1, thermal_4_1], dim=1)
        out_rgb_4 = self.idam_4(in_rgbt_4)
        fuse_4_1 = out_rgb_4[:,2048*2:2048*3,:]
        ######################################################################
        return [fuse_4_1,fuse_3_1,fuse_2_1,fuse_1,fuse]
...
class FATNet_pp(nn.Module):
    def __init__(self,n_class):
        super(FATNet_pp, self).__init__()
